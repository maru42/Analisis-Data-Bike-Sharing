...
@st.cache_data
def load_data(day_csv, hour_csv):
    day_data = pd.read_csv(day_csv)
    hour_data = pd.read_csv(hour_csv)
...
season_filter = st.sidebar.multiselect('Select Season', day_data['season'].unique(), day_data['season'].unique())
month_filter = st.sidebar.multiselect('Select Month', day_data['mnth'].unique(), day_data['mnth'].unique())

# Filter the data based on user selection
filtered_data = day_data[(day_data['season'].isin(season_filter)) & (day_data['mnth'].isin(month_filter))]
